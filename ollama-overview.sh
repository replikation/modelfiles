...
printf "${CYAN}%-35s${NC} ${BLUE}%-12s${NC} ${YELLOW}%-12s${NC} ${RED}%-12s${NC} ${GREEN}%-12s${NC}\n" "MODEL" "PARAMS" "QNT" "TEMP" "CTX LEN"
printf "${CYAN}%-35s${NC} ${BLUE}%-12s${NC} ${YELLOW}%-12s${NC} ${RED}%-12s${NC} ${GREEN}%-12s${NC}\n" "-----" "------" "---" "-------" "-------"
...
echo -e "${GREEN}CTX LEN:${NC} Context length (auto = managed by ollama) / Maximum possible context length of that model"
